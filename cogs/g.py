    ...
    while True:
      try:
        ...
        if not database.child("users").child(user.id).shallow().get().val():
          ...
          ##### CECILIA CHECKS IF CARD IS CECILIA CARD THAT ADDS CURRENCY. #####
        ...
        #### IF USER ID ALREADY EXISTS
          ...
          else: ##### IF STATUS IS ALREADY CLAIMED #####
            current_unix = int(time.time())

            ##### TIMESTAMP FROM DATABASE WITH ADDED 1800 SECONDS #####
